...
Public Class factura
    Public Shared Function dsfactura() As DataSet
    ...
    Private Shared Function crearTablaDetalle() As DataTable
        ...
        dt.Columns.Add(New DataColumn("operacion", GetType(Integer)))
        dt.Columns.Add(New DataColumn("factura", GetType(Integer)))
        ...
        Return dt
    End Function
    Private Shared Function crearTablafactura() As DataTable
        ...
        Return dt
    End Function
    Public Function recuperaCabeceras(ByVal esHistorial As Boolean, ByVal Proceso As String,
                ByVal esMensual As Boolean, ByVal fechaInicio As Date, ByVal fechaFinal As Date,
                ByVal nroDecimales As Integer, ByVal xAlmacen As Boolean, ByVal cod_alma As String,
                ByVal xVenta As Boolean, ByVal xCliente As Boolean, ByVal cod_clie As String, ByVal xDocumento As Boolean,
        ...
        cad3 = IIf(esHistorial, " from h_factura as h", " from factura as h")
    ...
    Public Function recuperaVentas(ByVal fechaI As Date, ByVal fechaF As Date, ByVal xarticulo As Boolean, ByVal xvendedor As Boolean, ByVal codvendedor As String) As DataSet
        ...
        cad1 = "select fecha,fec_doc,ser_doc,nro_doc,raz_soc,cliente.cod_clie,dir_clie,factura.monto as monto_doc,factura.tm,"
        cad2 = " factura.monto_igv, factura.pre_inc_igv,d.nro_maquina,nom_doc"
        ...
        cad4 = " from factura left join cliente on factura.cod_clie=cliente.cod_clie"
        cad5 = " inner join documento_s d on d.cod_doc=factura.cod_doc "
        cad6 = IIf(xarticulo, " inner join factura_det sd on factura.operacion=sd.operacion inner join articulo a on sd.cod_art=a.cod_art inner join subgrupo g on g.cod_sgrupo=a.cod_sgrupo", " inner join factura_detpago sdp on factura.operacion=sdp.operacion inner join forma_pago on sdp.cod_fpago=forma_pago.cod_fpago ")
        cad7 = " where fec_doc>='" & mfechaI & "'" & " and fec_doc<='" & mfechaF & "'" & IIf(xvendedor, " and factura.cuenta='" & codvendedor & "'", "")
    ...
    Public Function recuperaAtenciones(ByVal fechaI As Date, ByVal fechaF As Date, ByVal xGrupo As Boolean, ByVal xvendedor As Boolean, ByVal codvendedor As String) As DataSet
        ...
        cad1 = "select time(fecha) as hor_doc,fec_doc,raz_soc,cliente.cod_clie,dir_clie,fono_clie,factura.monto as monto_doc,factura.tm,"
        cad2 = " factura.monto_igv, factura.pre_inc_igv,d.nro_maquina,nom_doc,dsc_recurso as nom_atencion"
        ...
        cad4 = " from factura "
        cad5 = " left join cliente on factura.cod_clie=cliente.cod_clie"
        cad6 = " inner join documento_s d on d.cod_doc=factura.cod_doc inner join tipo_recurso tr on tr.cod_recurso=factura. cAux3 "
        cad7 = IIf(xGrupo, " inner join factura_det sd on factura.operacion=sd.operacion inner join articulo a on sd.cod_art=a.cod_art inner join subgrupo g on g.cod_sgrupo=a.cod_sgrupo", " inner join factura_detpago sdp on factura.operacion=sdp.operacion inner join forma_pago on sdp.cod_fpago=forma_pago.cod_fpago ")
        cad8 = " "
        cad9 = " where fec_doc>='" & mfechaI & "'" & " and fec_doc<='" & mfechaF & "'" & IIf(xvendedor, " and factura.cod_vend='" & codvendedor & "'", "")
    ...
    Public Function recuperaGrupoVentas(ByVal fechaI As Date, ByVal fechaF As Date, ByVal xGrupo As Boolean, ByVal xvendedor As Boolean, ByVal codvendedor As String) As DataSet
        ...
        cad4 = " from factura "
        cad5 = " left join cliente on factura.cod_clie=cliente.cod_clie"
        cad6 = " inner join documento_s d on d.cod_doc=factura.cod_doc "
        cad7 = IIf(xGrupo, " inner join factura_det sd on factura.operacion=sd.operacion inner join articulo a on sd.cod_art=a.cod_art inner join subgrupo g on g.cod_sgrupo=a.cod_sgrupo", " inner join factura_detpago sdp on factura.operacion=sdp.operacion inner join forma_pago on sdp.cod_fpago=forma_pago.cod_fpago ")
        cad8 = " "
        cad9 = " where fec_doc>='" & mfechaI & "'" & " and fec_doc<='" & mfechaF & "'" & IIf(xvendedor, " and factura.cod_vend='" & codvendedor & "'", "")
        cad10 = IIf(xGrupo, " group by a.cod_sgrupo order by a.cod_sgrupo ", "")
    ...
    Public Function recuperaCabecera(ByVal operacion As Integer, ByVal esPedido As Boolean) As DataTable
        Dim clConex As New MySqlConnection
        Dim da As New MySqlDataAdapter
        Dim ds As DataSet = factura.dsfactura
        Dim dt As DataTable = ds.Tables("cabecera")
        clConex = Conexion.obtenerConexion
        Dim com As New MySqlCommand
        Dim cad, cad1, cad2, cad3, cad4 As String
        If esPedido Then
            cad1 = "select factura.operacion,ope_ped,cod_doc,ser_doc,nro_doc,fec_doc,ser_ped,nro_ped,factura.cod_vend,nom_vend,cod_mesa,cod_atencion,"
            cad2 = " factura.cod_clie,raz_soc,dir_clie,factura.cod_fpago,factura.pre_inc_igv,factura.anul,factura.tm,factura.obs,factura.nro_ptovta"
            cad3 = " from factura left join pedido on factura.ope_ped=pedido.operacion left join cliente on factura.cod_clie=cliente.cod_clie"
            cad4 = " left join vendedor on factura.cod_vend=vendedor.cod_vend where factura.operacion=" & operacion
        Else
            cad1 = "select factura.operacion,cod_doc,ser_doc,nro_doc,'0' as ser_ped,'00000' as nro_ped,fec_doc,factura.cod_alma,nom_alma,obs,cod_vend,cod_mesa,cod_atencion,"
            cad2 = " factura.cod_clie,raz_soc,dir_clie,fono_clie,factura.cod_fpago,factura.pre_inc_igv,factura.tm,factura.nro_ptovta"
            cad3 = " from factura inner join cliente on factura.cod_clie=cliente.cod_clie inner join almacen on factura.cod_alma=almacen.cod_alma"
            cad4 = " where factura.operacion = " & operacion
        End If
    ...
    Public Function recuperaDetalle(ByVal operacion As Integer, ByVal nroDecimales As Integer) As DataTable
        ...
        'cad1 = "select factura,operacion,ingreso,factura_det.cod_art,nom_uni,cant as cantidad,If(LENGTH(obs) > 0,obs ,nom_art ) as nom_art,"
        cad1 = "select salida,operacion,ingreso,factura_det.cod_art,nom_uni,cant as cantidad,If(LENGTH(obs) > 0,obs ,nom_art ) as nom_art,"
        ...
        End If
        cad3 = " from factura_det inner join articulo on factura_det.cod_art=articulo.cod_art"
        cad4 = " inner join unidad on articulo.cod_uni=unidad.cod_uni"
        'cad5 = " where operacion=" & operacion & " order by factura"
    ...
    Public Function recuperaPago(ByVal operacion As Integer) As DataTable
        ...
        cad2 = " from factura s inner join factura_detpago sp on s.operacion=sp.operacion"
    ...
    Public Function recuperafactura(ByVal fechaI As Date, ByVal fechaF As Date) As DataSet
        Dim clConex As New MySqlConnection
        Dim da As New MySqlDataAdapter
        Dim ds As New DataSet
        clConex = Conexion.obtenerConexion
        ...
        cad1 = "select *,c.nom_clie,c.dir_clie from factura s inner join cliente c on s.cod_clie=c.cod_clie "
        ...
        Return ds
    End Function
    Public Function recuperafacturaDetalle(ByVal operacion As Integer) As DataSet
        Dim clConex As New MySqlConnection
        Dim da As New MySqlDataAdapter
        Dim ds As New DataSet
        clConex = Conexion.obtenerConexion
        Dim cad, cad1, cad2 As String
        cad1 = " select *  from factura_Det"
        ...
        Return ds
    End Function
    Public Function recuperafacturaPago(ByVal operacion As Integer) As DataSet
        Dim clConex As New MySqlConnection
        Dim da As New MySqlDataAdapter
        Dim ds As New DataSet
        clConex = Conexion.obtenerConexion
        Dim cad, cad1, cad2 As String
        cad1 = " select *  from factura_detpago"
        cad2 = " where operacion=" & operacion
        cad = cad1 + cad2
        Dim com As New MySqlCommand(cad)
        com.Connection = clConex
    ...
    Public Function insertar_factura_det(ByVal nro_ope As Integer,
        ...
        com.Connection = clConex
        Dim sql As String, result As Integer
        sql = "Insert Into factura_det(operacion,salida,ingreso,cod_art,imp_subtotal,imp_total,imp_igv,cant,precio,igv,descripcion,nAux)" &
            "values(" &
            nro_ope & "," & salida & "," & ingreso & ",'" & cod_art & "'," & impsubtotal & "," & imptotal & "," & impigv & "," & cant & "," & precio & "," & igv & ",'" & obs & "'," & nAux & ")"
        com.CommandText = sql
    ...
    Public Function insertar_aux(ByVal nro_ope As Integer,
        ...
        sql = "Insert Into factura(operacion,ope_ped,cod_doc,ser_doc,nro_doc,fec_doc,cod_vend,cod_clie,cod_fpago,cancelado,pre_inc_igv,nro_dec,cod_alma,cod_area,obs,cod_emp,cuenta,nAux,cAux,cAux2,cAux3,cAux4)" &
            "values(" &
        ...
        Return True
    End Function
    Public Function insertar_imp(ByVal nro_ope As Integer,
                        ByVal ope_ped As Integer,
                        ByVal fecha As DateTime,
                        ByVal cod_doc As String,
                        ByVal ser_doc As String,
        ...
        sql = "Insert Into factura(operacion,ope_ped,fecha,cod_doc,ser_doc,nro_doc,fec_doc,cod_vend,cod_clie,cod_fpago,cancelado,pre_inc_igv,nro_dec,cod_alma,cod_area,obs,cod_emp,cuenta,nAux,cAux,cAux2)" &
            "values(" &
    ...
    Public Function insertar_detObs(ByVal nro_ope As Integer,
        ...
        com.Connection = clConex
        Dim sql As String, result As Integer
        sql = "Insert Into factura_det(operacion,factura,ingreso,cod_art,cant,precio,igv,comi_v,comi_jv,obs,nAux)" &
            "values(" &
            nro_ope & "," & factura & "," & ingreso & ",'" & cod_art & "'," & cant & "," & precio & "," & igv & "," & cv & "," & cv1 & ",'" & obs & "'," & nAux & ")"
        com.CommandText = sql
        result = com.ExecuteNonQuery
        clConex.Close()
        Return result
    ...
    Public Function insertar_detPago(ByVal nro_ope As Integer,
                            ByVal cod_fpago As String,
                            ByVal imp_pagado As Decimal,
                            ByVal imp_propina As Decimal,
                            ByVal imp_vuelto As Decimal,
        ...
        com.Connection = clConex
        Dim sql As String, result As Integer
        sql = "Insert Into factura_detPago(operacion,cod_fpago,imp_pagado,imp_propina,imp_vuelto,obs_pago)" &
            "values(" &
    ...
    Public Function Actualiza_montopago(ByVal nro_ope As Integer, ByVal monto As Decimal) As Boolean
        ...
        com.Connection = clConex
        Dim sql As String, result As Integer
        sql = "update factura set monto_pago= " & monto & " where operacion=" & nro_ope
        com.CommandText = sql
        result = com.ExecuteNonQuery
        clConex.Close()
        Return result
    ...
    Public Function Actualiza_cambioMesa(ByVal nro_ope As Integer, ByVal mesa As String) As Boolean
        ...
        com.Connection = clConex
        Dim sql As String, result As Integer
        sql = "update factura set Caux2= '" & mesa & "' where operacion=" & nro_ope
        com.CommandText = sql
        result = com.ExecuteNonQuery
        clConex.Close()
        Return result
    ...
    Public Function Actualiza_DetcambioMesa(ByVal nro_opeX As Integer, ByVal nro_Ope As Integer, ByVal ingreso As Integer) As Boolean
        ...
        sql = "update factura_det set operacion= " & nro_opeX & ", ingreso = ingreso + 100  where operacion=" & nro_Ope & " and ingreso= " & ingreso

        com.CommandText = sql
        result = com.ExecuteNonQuery
        sql1 = "update factura_det set cant = cant  where operacion=" & nro_opeX
    ...
    Public Function AnularDetalle(ByVal eshis As Boolean,
                                 ByVal proceso As String,
                                 ByVal nroOperacion As Integer,
                         ByVal nroIngreso As Integer,
        ...
        Dim cad, cad1, cad2 As String
        cad1 = "update " & IIf(eshis, "h_factura_det", "factura_det") & " set cant= cant*" & factor1 & ",nAux= " & factor2
        ...
        Return result
    End Function
    Public Function Actualiza_documento(ByVal nro_ope As Integer, ByVal lcancelado As Integer, ByVal fecha_pago As Date, ByVal cod_doc As String, ByVal Caux As String, ByVal ser_doc As String, ByVal nro_doc As String,
        ...
        sql = "update factura set cancelado=" & lcancelado & ",fec_pago='" & mfecha & "',cod_doc= '" & cod_doc & "', ser_doc= '" & ser_doc & "',nro_doc='" & nro_doc & "',cuenta='" & cod_user & "', " _
    ...
    Public Function eliminaItem(ByVal nro_ope As Integer, ByVal nroIngreso As Integer) As Boolean
        ...
        cad = "delete from factura_det where operacion=" & nro_ope & " and ingreso=" & nroIngreso
        com.CommandText = cad
        result = com.ExecuteNonQuery
        clConex.Close()
        Return result
    ...
    Public Function maxOperacion_fac() As Integer
        ...
        Return result + 1
    End Function
    Public Function maxfactura() As Integer
        Dim clConex As MySqlConnection = Conexion.obtenerConexion
        Dim com As New MySqlCommand, result As Integer
        com.Connection = clConex
    ...
    Public Function maxIngreso(ByVal nro_ope As Integer) As Integer
        ...
        Dim com As New MySqlCommand, result As Integer
        com.Connection = clConex
        com.CommandText = "select max(ingreso) from factura_det where operacion=" & nro_ope
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), 0, obj), Integer)
        clConex.Close()
        Return result + 1
    ...
    Public Function maxPedido() As Integer
        ...
        Dim com As New MySqlCommand, result As Integer
        com.Connection = clConex
        com.CommandText = "select max(ope_ped) from factura"
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), 0, obj), Integer)
        clConex.Close()
        Return result + 1
    ...
    Public Function existeDocumento(ByVal cod_doc As String, ByVal ser_doc As String, ByVal nro_doc As String, ByVal tipconex As Integer) As Integer
        ...
        End If
        com.Connection = clConex
        sql = "Select operacion from factura where cod_doc='" & cod_doc & "' and ser_doc='" & ser_doc & "'" _
                & " and nro_doc='" & nro_doc & "'"
        com.CommandText = sql
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), 0, obj), Integer)
    ...
    Public Function existeOperacion(ByVal operacion As Integer, ByVal tipconex As Integer) As Boolean
        ...
        End If
        com.Connection = clConex
        sql = "Select operacion from factura where operacion=" & operacion
        com.CommandText = sql
        Dim obj As Object = com.ExecuteScalar
    ...
    Public Function buscarMesa(ByVal cod_mesa As String, ByVal fecha As Date) As Integer
        ...
        com.Connection = clConex
        sql = "Select operacion from factura where LENGTH(nro_doc)=0 and fec_doc='" & mfechaI & "' and cod_mesa='" & cod_mesa & "'"
        com.CommandText = sql
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), 0, obj), Integer)
        clConex.Close()
    ...
    Public Function buscarPedido(ByVal nro_pedido As Integer) As Integer
        ...
        Dim com As New MySqlCommand, result As Integer, sql As String
        com.Connection = clConex
        sql = "Select operacion from factura where LENGTH(nro_doc)=0 and ope_ped=" & nro_pedido
    ...
    Public Function buscarOperacionPedido(ByVal nro_operacion As Integer) As Integer
        ...
        com.Connection = clConex
        sql = "Select ope_ped from factura where  operacion=" & nro_operacion
        com.CommandText = sql
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), 0, obj), Integer)
        clConex.Close()
    ...
    Public Function documento(ByVal nro_ope As Integer) As String
        ...
        com.Connection = clConex
        cad = "Select nro_doc from factura where operacion='" & nro_ope & "'"
        com.CommandText = cad
        Dim obj As Object = com.ExecuteScalar
    ...
    Public Function usuario(ByVal nro_ope As Integer) As String
        ...
        Dim com As New MySqlCommand, result As String, cad As String
        com.Connection = clConex
        cad = "Select cuenta from factura where operacion='" & nro_ope & "'"
        com.CommandText = cad
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), False, obj), String)
        clConex.Close()
    ...
    Public Function fechadoc(ByVal nro_ope As Integer) As Date
        ...
        com.Connection = clConex
        cad = "Select fec_doc from factura where operacion='" & nro_ope & "'"
        com.CommandText = cad
        Dim obj As Object = com.ExecuteScalar
    ...
    Public Function estaAnulada(ByVal nro_ope As Integer) As Boolean
        ...
        com.Connection = clConex
        cad = "Select anul from factura where operacion='" & nro_ope & "'"
        com.CommandText = cad
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), False, obj), Integer)
        clConex.Close()
    ...
    Public Function maxNrofactura(ByVal cod_doc As String, ByVal ser_doc As String) As String
        Dim clConex As MySqlConnection = Conexion.obtenerConexion
        Dim com As New MySqlCommand, result As String
        com.Connection = clConex
        com.CommandText = "select max(nro_doc) from factura where cod_doc='" & cod_doc & "'" & " and ser_doc='" & ser_doc & "'"
        Dim obj As Object = com.ExecuteScalar
        ...
        Return resp
    End Function
    Public Function existeDocumentoRef(ByVal cod_doc As String, ByVal ser_doc As String, ByVal nro_doc As String, ByVal tipconex As Integer) As Integer
        Dim clConex As MySqlConnection
        Dim com As New MySqlCommand, result As Integer, sql As String
        If tipconex = 0 Then
            clConex = Conexion.obtenerConexion()
        ...
        End If
        com.Connection = clConex
        sql = "Select operacion from factura where cod_doc_ref='" & cod_doc & "' and ser_doc_ref='" & ser_doc & "'" _
                & " and nro_doc_ref='" & nro_doc & "'"
        com.CommandText = sql
        Dim obj As Object = com.ExecuteScalar
        result = CType(IIf(IsDBNull(obj), 0, obj), Integer)
        clConex.Close()
